 ...
 * Handles iOS Safari quirks with date string formats (e.g. "YYYY-MM-DD HH:mm:ss").
 */
export function parseDateSafe(input?: string | number | Date | null): Date {
  ...
  if (typeof input === 'string') {
    ...
    // 1. Standard Date parsing
    let parsed = new Date(trimmed);
    if (!isNaN(parsed.getTime())) {
      return parsed;
    }

    // 2. Safari fix: Space-separated ISO dates (e.g. "2026-08-18 15:04:00" -> "2026-08-18T15:04:00")
    ...
    // 3. Safari fix: Replace dashes with slashes (e.g. "2026-08-18" -> "2026/08/18")
    if (trimmed.includes('-')) {
